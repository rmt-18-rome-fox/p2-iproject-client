<template>
  <div class="bg-gray-200 py-4 justify-center px-4 rounded-xl shadow-md">
    <div class="flex-1 mx-auto">
      ...
      <button v-if="anime.status == 'Watched'" class="bg-red-400 ml-4 py-2 px-1 rounded-md shadow-md" @click.prevent="deleteWatchList(anime.JikanAnimeId, anime.title)">Delete</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    deleteWatchList(JikanAnimeId, title){
      ...
      .then((res) => {
        swal.fire(`Success Delete ${title} from WatchLists`)
        this.$store.dispatch("fetchWatchLists")
      })
    }
  }
