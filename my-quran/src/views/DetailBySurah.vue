...
<div>
    ...
    <div class="py-5">
        ...
        <div class="flex justify-center w-10/12 my-10">
            <button class="mx-2 rounded-full px-2 py-1 bg-green-500 text-white">{{detailSurah.pagination.current_page}}</button>
            <button v-show="detailSurah.pagination.next_page !== null" class="mx-2 rounded-full px-2 py-1 bg-green-500 text-white">{{detailSurah.pagination.next_page}}</button>
        </div>
    </div>
</div>
</template>
